...
def _rolling_circle_slices(mesh, seed_pt, locs, dir, thresh):
    end_pts = []
...
def _distal_proximal_zs_articular(end_pts):
...
def _inf_sup_articular(end_pts, minor_axis):
    # the end points alternate back and forth so seperate them out
    _, labels, _ = sklearn.cluster.k_means(end_pts, 2)
    pts0 = end_pts[np.where(labels == 0)]
    pts1 = end_pts[np.where(labels == 1)]
...
def plane(
    ...
    hc_mnr_line, hc_mnr_length = _midpoint_line(gt_side_pt, non_gt_side_pt)
    ...
    hc_mnr_end_pts = _rolling_circle_slices(
        mesh_csys,
        seed_pt,
    ...
    inf_pts, sup_pts = _inf_sup_articular(hc_mnr_end_pts, hc_mnr_axis)
    ...
    inf_sup_line, inf_sup_len = _midpoint_line(inf_mean, sup_mean)
    inf_sup_cut_locs = np.linspace(
        inf_sup_line.to_point(t=-inf_sup_len / 6),
        inf_sup_line.to_point(t=inf_sup_len / 6),
        med_lat_num,
    )
    med_lat_pts = _rolling_circle_slices(
        mesh_csys, seed_pt, inf_sup_cut_locs, inf_sup_line.direction, circle_threshold
    )
    ...
    fit_plane_pts = utils.transform_pts(
        fit_plane_pts, utils.inv_transform(transform)
    ...
    plane_trace = np.array(
        mesh.section(plane_origin=plane.point, plane_normal=plane.normal).vertices
    )
    plane_pts = plane.to_points(
        lims_x=(-30, 30), lims_y=(-30, 30)
